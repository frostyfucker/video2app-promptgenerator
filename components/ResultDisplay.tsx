...
import { CopyIcon, CheckIcon, RefreshIcon, WandIcon, LinkIcon, SparklesIcon } from './icons';
...
export const ResultDisplay: React.FC<ResultDisplayProps> = ({ prompt, onReset, sources, onRefine }) => {
  ...
  return (
    <div className="bg-brand-surface border border-brand-border rounded-xl shadow-lg animate-fade-in">
      ...
      <div className="p-4 sm:p-6 border-t border-brand-border bg-gray-800/20">
          <h3 className="flex items-center text-lg font-semibold text-brand-text mb-4">
          ...
          <form onSubmit={handleRefineSubmit} className="space-y-3">
            <textarea
              ...
              placeholder="e.g., 'Make the tech stack more beginner-friendly' or 'Add a feature for user profiles.'"
              className="w-full px-4 py-2 bg-brand-bg border border-brand-border rounded-lg focus:outline-none focus:ring-2 focus:ring-brand-primary text-sm text-brand-text-secondary"
              rows={3}
            ...
            <button
              ...
              className="px-4 py-2 w-full sm:w-auto font-semibold text-white bg-brand-primary rounded-lg hover:bg-brand-primary-hover focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-brand-surface focus:ring-brand-primary disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
            >
              Refine Prompt
            </button>
          </form>
